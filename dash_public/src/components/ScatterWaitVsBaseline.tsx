// src/components/ScatterWaitVsBaseline.tsx
...
export const ScatterWaitVsBaseline: React.FC<Props> = ({ rows }) => {
  const data = useMemo(
    () =>
      rows.map((r) => ({
        id: r.pair,
        baseline: r.baseline_wait,
        rl: r.rl_wait,
      })),
    [rows]
  );
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#E5E7EB] shadow-sm p-5">
      ...
      <p className="text-xs text-[#6B7280] mb-4">
        Points below the diagonal mean RL achieved lower delay than the
        baseline (good).
      </p>
      <div className="h-56">
        <ResponsiveContainer>
          <ScatterChart margin={{ top: 10, right: 20, bottom: 20, left: 0 }}>
            ...
            <XAxis
              ...
              tick={{ fontSize: 11 }}
              label={{ value: "Baseline (s)", position: "insideBottom", dy: 12 }}
            />
            <YAxis
              type="number"
              dataKey="rl"
            ...
            <ReferenceLine
              ...
              strokeDasharray="4 4"
            />
            <Scatter data={data} fill="#22C55E" opacity={0.85} />
            <Tooltip
